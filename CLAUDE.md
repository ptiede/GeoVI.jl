...
- `transform_block(family, lh, θ, noise_i, mirrored)` — the **frozen prefix**, run at the current `θ` and held constant, producing a stored residual. Trivial (`= ε`) for pushforward families; a CG solve (MGVI) or CG + nonlinear curve (geoVI) for the Fisher-Gaussian families. Expensive, not differentiated, reusable for CRN.
- `transport(family, θ, r)` — the **differentiated suffix**, reconstructing `ξ` from `θ` + the stored residual. Default `θ .+ r` (MGVI/geoVI: `θ` *is* the latent point); mean-field: `μ + σ⊙ε`; a flow: `T_θ(ε)`.

Everything else *follows* from the cut: freezing the covariance ⇒ the `log q` term (`logdensity(family, θ, ε)`, default `0`) vanishes (MGVI/geoVI's fixed-metric approximation); differentiating the shape ⇒ it carries `-Σlogσ` (mean-field) / `-log|det J|` (flow). The natural-gradient metric (`natural_gradient_metric`) is the frozen covariance `(I+Fisher)` reused as the `NewtonCG` preconditioner — an *optimizer* concern (consulted only by `NewtonCG`, gated by `supports_natural_gradient`), not a divergence one.

**Parameter container vs latent point.** The *variational parameters* `θ` (what the optimizer moves; `VIState.position`) are decoupled from the *latent point* `ξ` (what the likelihood consumes — a flat white array). For MGVI/geoVI they coincide (`θ` *is* the latent mean, so the `transport` default applies and no override is needed). Mean-field's `θ = (; mean, logstd)` is a NamedTuple, so it overrides `init_params`/`transport`/`logdensity` (the σ-scaling lives in `transport` to stay differentiable). `θ` is a Functors-compatible container, so `Optimisers.setup`/`update` and Enzyme/Reactant AD handle it natively; latent-space machinery (residual/noise buffers, `_sample_slice`, the `0.5·dot(ξ,ξ)` prior, the `@trace` MC loops) stays array-based. Tree-generic helpers (`_param_sub`/`_param_norm`/`_param_all_finite` in `src/tree_utils.jl`, `fmap(copyto!, …)` in `update!`, `Optimisers.destructure` for AutoFiniteDiff on a NamedTuple) cover the `θ`-space ops. There is no `mean` / `latent_mean` / `latent_template`; the fitted output is a *pure* distribution (`rand`, with no retained samples — draw fresh ones), and all buffers are sized from the user's initial `ξ₀`.
...
Hot loops use `ReactantCore.@trace` so they run under both eager and traced execution. This spans `src/cg.jl` (inner CG), `src/optimize.jl` (Newton/line-search loops), `src/vi.jl` (the n-sample draw loop, the `transform!` de-whitening loop, and the Monte Carlo divergence sum), `src/families/fisher_gaussian.jl` (the natural-gradient metric's MC sum), and `ext/GeoVIReactantExt.jl`. Conventions to keep:
...
- **divergence** (`ReverseKL`; `ForwardKL` is a stub) — the objective form. `_fdivergence_value` dispatches jointly on `(family, divergence)` and is the single reverse-KL objective for *every* family (`mean_i[-log p(ξ_i) + logdensity(family, θ, r_i)]`, the family's `log q_θ`). The natural-gradient metric is **not** on this axis — it is `natural_gradient_metric(family, lh, θ, residuals, v)`, an optimizer concern used only by `NewtonCG`.
...
- Custom variational families (the families live in `src/families/`: `interface.jl` defines the contract + all generic defaults, then one file per family — `mgvi.jl`, `geovi.jl`, the shared `fisher_gaussian.jl`, `meanfield.jl`): subtype `AbstractVariationalFamily`, implement `init_params` + `transport` (the latter only if `θ` is not itself the latent point — its default is `θ .+ r`). Each other hook has a default targeting the pushforward case: a typical pushforward family (mean-field, normalizing flow) adds only `logdensity(family, θ, ε)` (the family's log-density `log q_θ`, extending the likelihood's `logdensity`; default `0`); a structured `θ` also overrides `draw_noise` (to back `rand`); a family with a non-trivial frozen draw overrides `transform_block` + `init_noise` (default: one latent-shaped ε buffer); and to enable `NewtonCG`, set `supports_natural_gradient(family) = true` and implement `natural_gradient_metric`. MGVI/geoVI share one implementation via the `FisherGaussian` union and differ only in `_refine_residual` (geoVI adds the curve)
...
q = fit(problem, 8; rng=MersenneTwister(42), n_refine=0)
