...
"""
struct MGVIFamily{S, P} <: AbstractVariationalFamily
    solver::S
    strict::Bool
    preconditioner::P
end
...
# default. `preconditioner` (default `nothing`) is the draw-solve preconditioner — pass a
# [`JacobiPreconditioner`](@ref) when the data Fisher is stiff/ill-conditioned.
MGVIFamily(; solver = ConjugateGradient(rtol = 1.0e-5, miniter = 6), strict = false, preconditioner = nothing) =
    MGVIFamily(solver, strict, preconditioner)

# The frozen draw refinement (see `draw_samples!` in `fisher_gaussian.jl`): MGVI keeps
# the linear CG residual as-is.
_refine_residual(::MGVIFamily, lh, μ, linear, ms, mirrored, precond = nothing) =
    mirrored ?
